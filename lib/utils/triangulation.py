            ...
            for keypoint_idx in range(self.num_keypoints):
                ...
                if len(points) >= self.min_views:
                    ...
                    if debug == True and use_secondary == False:
                        print('kps_idx:{} kps_name:{} kps_error:{}, inliers:{}, {}'.format(keypoint_idx, COCO_KP_ORDER[keypoint_idx], reprojection_error_vector.mean(), len(inlier_views), [choosen_cameras[index][0] for index in inlier_views]))
