...
##------------------------------------------------------------------------------------
class ExoCamera:
    def __init__(self, cfg, root_dir, colmap_dir, exo_camera_name='cam01', coordinate_transform=None, exo_camera_mapping=None, max_time_stamps=4):
        self.cfg = cfg
        ...
        self.reconstruction = pycolmap.Reconstruction(self.colmap_dir)
    ...
    # def cam_from_world_deprecated(self, point_3d):
    #     assert(point_3d.shape[0] == 3)
    #     point_3d_prime = (linear_transform(points_3d=np.expand_dims(point_3d, axis=0), T=self.coordinate_transform))[0]

    #     # -----------world to camera coordinates-----------
    #     point_3d_cam = self.extrinsics_image.project(point_3d_prime) ## using pycolmap

    #     # ##-----------add a third dimenison depending on if the point is behind the camera
    #     if self.check_point_behind_camera_3d(point_3d):
    #         point_3d_cam = np.array([point_3d_cam[0], point_3d_cam[1], -1])
    #     else:
    #         point_3d_cam = np.array([point_3d_cam[0], point_3d_cam[1], 1])
    ...
    ## to camera coordinates 3D from world cooredinate 3D
    ### TO DO: Scaling issue here, needs to be fixed.
    # # ## work in progress, scaling iisues
    ...
    def check_point_outside_frustrum(self, point_3d):
        ...
        # if not self.frustum_polygon.contains(Point(p_prime_vec)):
        #     return True
        
        # for ray in self.frustum_rays:
        #     ## outside the frustrumr
        #     if np.dot(ray, point_3d) < 0:
        #         return True
    ...
    def image_from_cam(self, point_3d, eps=1e-9):
        ...
        # if self.check_point_outside_frustrum(point_3d):
        #     return np.array([-1, -1]) ## out of bound
    ...
    def vec_project(self, point_3d):
        ...
        # return point_2d
        return self.project_batch(point_3d)
    ...
    # # the intrinsics paramters are // intrinsic = ##----"fx, fy, cx, cy, k1, k2, p1, p2";-----
    # # k_1, k_2, are radial distortion coefficients. p_1 and p_2 are tangential distortion coefficients.
    # def vec_image_from_cam(self, point_3d, eps=1e-9):
    #     all_point_2d = []

    #     for idx in range(len(point_3d)):
    #         import pdb; pdb.set_trace()       
    #         point_2d = self.intrinsics.world_to_image(point_3d[idx].reshape(-1, 1))
    #         all_point_2d.append(point_2d)

    #     import pdb; pdb.set_trace()
    #     point_2d = np.concatenate(all_point_2d, axis=0)    
